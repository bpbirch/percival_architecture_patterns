...
class Batch(BaseModel):
    ...
    def can_allocate_by_qty(self, line: OrderLine) -> bool:
        total_allocations = line.qty + sum([order_line.qty for order_line in self.allocations])
        return self.purchased_quantity >= total_allocations

    def allocate(self, line: OrderLine):
        if not self.can_allocate_by_sku(line):
            raise SkuMismatch(f'line sku of {line.sku} does not match batch sku of {self.sku}')
        if not self.can_allocate_by_qty(line):
            raise OutOfStock(f'line qty of {line.qty} is larger than batch available quantity of {self.available_quantity}')
    ...
    def deallocate(self, line: OrderLine):
        if self.can_deallocate(line):
            self.allocations = [order_line for order_line in self.allocations if order_line != line]
