...
class BatchTester(unittest.TestCase):
    ...
    def test_can_allocate_by_sku(self) -> None:
        self.assertTrue(
            self.matching_batch.can_allocate_by_sku(self.matching_line) == True
        )
        self.assertFalse(
            self.mismatch_sku_batch.can_allocate_by_sku(self.mismatch_sku_line) == True
        )

    def test_can_allocate_by_qty(self) -> None:
        self.assertTrue(
            self.matching_batch.can_allocate_by_qty(self.matching_line) == True
        )
        self.assertFalse(
            self.mismatch_qty_batch.can_allocate_by_qty(self.mismatch_qty_line) == True
        )
